...
before(async () => {
  ...
  // For the comment checks below: one page built with html``, and one plain-string body that must NOT be touched.
  // The difference is the point — html`` is HTML by construction, a string body might be JSON.
  app.get("/withcomment", ({ res }) => send(res, 200, html`<p>before</p>${raw("<!-- developer prose -->")}<p>after</p>`));
  app.get("/notHtml", ({ res }) => send(res, 200, JSON.stringify({ name: "Ann <!-- note --> Berg" }),
...
// ---- comments do not reach the wire, and only HTML is touched --------------------------------------------------
//
...
//
// The SECOND assertion is a regression I shipped and then measured. Stripping in send() touched every payload,
// including the JSON export — which is a data-protection right. A volunteer named "Ann <!-- note --> Berg" came out
// as "Ann  Berg": valid JSON, silently not their record. Doing it in html`` means only HTML is ever affected.
test("html`` drops comments; a non-HTML body is sent byte for byte", async () => {
  ...
  // The control: the content either side must survive, or "no comment" is also satisfied by an empty response.
  ...
  // A body that is NOT HTML keeps every byte, including something that looks like a comment inside a value.
  const json = await (await fetch(`${base}/notHtml`)).text();
  assert.equal(JSON.parse(json).name, "Ann <!-- note --> Berg",
    "an export must match the record — this is the regression that stripping in send() caused");
});
